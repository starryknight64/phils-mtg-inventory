...
class BootStrap {
	...
	def init = { servletContext ->
		...
		def mtgImage = new HTTPBuilder("http://mtgimage.com/")
		def setSymbols = mtgImage.get( path: "SetSymbols.json", contentType: ContentType.JSON )
		def mtgImageSymbolsHash = Meta.findByName("mtgImageSymbolsHash") ?: new Meta(name:"mtgImageSymbolsHash",value:"").save()
		if( mtgImageSymbolsHash.value != "${setSymbols.hashCode()}" ) {
			print "New MTG Set Symbols were added! Updating now..."			
			mtgImageSymbolsHash.value = "${setSymbols.hashCode()}"
			mtgImageSymbolsHash.save()
			
			setSymbols.each{ setCode, rarities ->
				Expansion expansion = Expansion.findByCode(setCode)
				if( expansion ) {
					def expRarities = rarities.size() > 0 ? "" : null
					rarities.each{ rarity ->
						expRarities += rarity
					}
					expRarities = expRarities?.toLowerCase()
					if( expansion.rarities?.toLowerCase() != expRarities ) {
						expansion.rarities = expRarities
						expansion.save()
					}
				}
			}
