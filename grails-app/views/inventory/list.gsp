			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="name" title="${message(code: 'inventory.name.label', default: 'Name')}" />
				...
				<g:each in="${inventoryInstanceList}" status="i" var="inventoryInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${inventoryInstance.id}">${fieldValue(bean: inventoryInstance, field: "name")}</g:link></td>
